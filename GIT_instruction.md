...
```
Иногда может потребоваться авторизация (например, если работаешь с несколькими аккаунтами github на одном устройстве). Команда:
```
git clone git@github.com:<имя_пользователя>/<адрес_репозитория>.git
```
...
#### 6.4.1. Разрешение конфликтов при работе в команде (error: Your local changes to the following files would be overwritten by merge:)

Такого рода конфликты могут быть разрешены двумя способами - без сохранения локальных изменений (если они не нужны) или с сохранением. Рассмотрим сначала первый вариант.

1. Слияние репозиториев без сохранения локальных изменений

Сначала нам нужно "припрятать" локальные изменения, потом дропнуть их, а после спокойно делать pull:
```
... $ git stash
